...
const GameDetail = ({ pathId }) => {
    ...
    const getStar = () => {
        ...
        for (let i = 1; i <= 5; i++) {
            if (i <= rating) {
                star.push(<img src={starFull} key={i} alt='starFull' />)
            } else {
                star.push(<img src={startEmpty} key={i} alt='starEmpty' />)
            }
        }
        return star;
    }
    // Toggle from gameDetail to Home
                ...
                <CardShadow className='shadow' onClick={exitDetailHandler}>
                    <Detail layoutId={pathId}>
                        ...
                        <div className="gallery">
                            {screen.results.map(state => (
                                <img src={smallImage(state.image, 1280)} key={state.id} alt={state.image} />
                            ))}
                        </div>
                    </Detail>
                </CardShadow>
